...
echo -e "\033[93m"
echo "    reference pretty  =  reference"
echo "    ======================"
echo -e -n "\033[0m"
COUNT=0
COUNT_PASSED=0
...
then
  ...
  do
    ...
    if ( ./run.sh pretty $PROGRAM > tmp1 2>/dev/null &&
      ~cs520/golitec parse < tmp1 > tmp2 &&
      ~cs520/golitec parse < $PROGRAM > tmp1 &&
      diff tmp1 tmp2
    ) then
    ((COUNT_PASSED++))
    else
  ...
  echo -e "\e[${STATUS_COLOUR}m# reference pretty = reference: ${COUNT_PASSED}/${COUNT}\e[49m"
  RESULTS+=("\e[${STATUS_COLOUR}m# reference pretty = reference: ${COUNT_PASSED}/${COUNT}\e[49m")
else 
